...
func _get_route_square_ids():
	if self.player_id == Globals.ePlayer.YELLOW:
		return [101] + Array(range(5, 51)) + Array(range(1, 5)) + Array(range(51, 59 + 1))
	elif self.player_id == Globals.ePlayer.BLUE:
		return [102] + Array(range(22, 51)) + Array(range(1, 22)) + Array(range(60, 67 + 1))
	elif self.player_id == Globals.ePlayer.RED:
		return [103] + Array(range(39, 51)) + Array(range(1, 39)) + Array(range(68, 75 + 1))
	return []
